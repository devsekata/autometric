        ...
        <div className="px-6 py-5 flex flex-col gap-5">
          ...
          {!selected ? (
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full h-10 pl-3 pr-9 text-[13.5px] text-[#111827] placeholder:text-[#d1d5db] bg-white border border-[#e5e7eb] rounded-lg outline-none focus:border-[#3d7e96] focus:ring-2 focus:ring-[#3d7e96]/10 transition-all"
                />
                {/* Right icon: spinner or clear */}
                <div className="absolute right-2.5 top-1/2 -translate-y-1/2">
                  {searchState === 'loading' ? (
                    <div className="w-4 h-4 border-2 border-[#3d7e96]/30 border-t-[#3d7e96] rounded-full animate-spin" />
                  ) : email.length > 0 ? (
              ...
              {searchState === 'found' && searchResult && (
                <button
                  ...
                  className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg border border-[#e5e7eb] hover:border-[#3d7e96] hover:bg-[#f0f7fa] transition-all text-left mt-0.5"
                >
                  ...
                  <span className="text-[11px] font-semibold text-[#3d7e96] flex-shrink-0">Select</span>
                </button>
              )}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg border border-[#3d7e96] bg-[#f0f7fa]">
                ...
                {ROLE_OPTIONS.map(opt => (
                  <button
                    ...
                    className={`flex-1 flex flex-col items-start gap-0.5 px-3 py-2.5 rounded-lg border text-left transition-all ${
                      role === opt.value
                        ? 'border-[#3d7e96] bg-[#f0f7fa]'
                        : 'border-[#e5e7eb] hover:border-[#c5dce5] hover:bg-[#fafafa]'
                    }`}
                  >
                    <span style={PJB} className={`text-[13px] font-semibold ${role === opt.value ? 'text-[#1e6278]' : 'text-[#374151]'}`}>
                      {opt.label}
                    </span>
