      ...
      <div className="relative bg-white rounded-xl w-full max-w-[460px] mx-4 shadow-xl shadow-black/8 border border-[#e5e7eb]">
        ...
        <div className="px-6 py-5 flex flex-col gap-5">
          ...
          {!selected ? (
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full h-10 pl-3 pr-9 text-[13.5px] text-[#111827] placeholder:text-[#d1d5db] bg-white border border-[#e5e7eb] rounded-lg outline-none focus:border-[#285D6E] focus:ring-2 focus:ring-[#285D6E]/10 transition-all"
                />
                {/* Right icon: spinner or clear */}
                <div className="absolute right-2.5 top-1/2 -translate-y-1/2">
                  {searchState === 'loading' ? (
                    <div className="w-4 h-4 border-2 border-[#285D6E]/30 border-t-[#285D6E] rounded-full animate-spin" />
                  ) : email.length > 0 ? (
              ...
              {searchState === 'found' && searchResult && (
                <button
                  ...
                  className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg border border-[#e5e7eb] hover:border-[#285D6E] hover:bg-[#f0f7fa] transition-all text-left mt-0.5"
                >
                  ...
                  <span className="text-[11px] font-semibold text-[#285D6E] flex-shrink-0">Select</span>
                </button>
              )}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-3 px-3.5 py-2.5 rounded-lg border border-[#285D6E] bg-[#f0f7fa]">
                ...
                {ROLE_OPTIONS.map(opt => (
                  <button
                    ...
                    className={`flex-1 flex flex-col items-start gap-0.5 px-3 py-2.5 rounded-lg border text-left transition-all ${
                      role === opt.value
                        ? 'border-[#285D6E] bg-[#f0f7fa]'
                        : 'border-[#e5e7eb] hover:border-[#c5dce5] hover:bg-[#fafafa]'
                    }`}
                  >
                    <span style={PJB} className={`text-[13px] font-semibold ${role === opt.value ? 'text-[#327488]' : 'text-[#374151]'}`}>
                      {opt.label}
                    </span>
        ...
        <div className="px-6 py-4 flex items-center justify-end gap-2">
          ...
          <button
            ...
            style={PJB}
            className="h-8 px-4 bg-[#285D6E] hover:bg-[#1E4A58] disabled:opacity-40 text-white text-[13px] font-semibold rounded-lg transition-colors flex items-center gap-1.5"
          >
            {submitting ? 'Sending…' : (
              <>
