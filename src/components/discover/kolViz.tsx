...
/**
 * The estimate marker, currently switched off at the owner's request.
 *
 * It used to stamp every figure the warehouse cannot back, adjacent to the
 * number rather than in a legend, so the qualification travelled with a
 * screenshot. That is no longer wanted on screen, and this is the one place that
 * decision lives: rendering null here clears the marker everywhere at once.
 *
 * Deliberately switched off *here* rather than by deleting the call sites. Every
 * `sample={…}` prop and every flag in `CreatorIntel.real` is still computed and
 * still correct, so the product keeps knowing which figures are measured and
 * which are modelled — turning the marker back on is flipping `SHOW_MARKER`, not
 * re-deriving provenance across a dozen components.
 *
 * The switch is not what keeps generated numbers off the screen — removing
 * their source is. `@/lib/discover/kolSample` has been deleted, and the creator
 * workspace now renders a measured value or "Belum terukur" for every figure,
 * so no call site in it passes `sample` any more.
 */
const SHOW_MARKER = false

export function SampleTag({ compact, basis }: { compact?: boolean; basis?: string }) {
  if (!SHOW_MARKER) return null
  return (
    <span
      title={basis
        ? `Estimasi — ${basis}. Belum ada sumber terukurnya di database KOL.`
        : 'Estimasi — dihitung dari followers dan engagement rate creator ini, '
          + 'bukan angka terukur. Belum ada sumber datanya di database KOL.'}
