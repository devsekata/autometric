...
export interface SectionProps {
   ...
   * saved Brand Profile. Null while loading, and null when no profile is saved
      ...
      <VizCard title="Performance Overview" subtitle="Rata-rata per konten">
        <div className="grid gap-2.5" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))' }}>
          ...
          <StatTile label="Views (rata-rata)"
            ...
            hint={intel.kpi.avgViews === null ? undefined : basis} />
      ...
      <Split
        ...
        aside={
          <>
            ...
            {/* Posting cadence, computed from the real first/last post dates
                across the harvest. Null below two posts or a window under a day
                — a cadence from one post is arithmetic, not a rate. */}
            <VizCard title="Frekuensi Posting" subtitle={measuredBasis(intel)}>
              {intel.content.postsPer30d === null
                ? <Unavailable text="Butuh minimal dua post terpanen dengan rentang tanggal untuk menghitung frekuensi." />
                : (
                  <div style={{ ...PJ, color: T.t1 }} className="text-[20px] font-extrabold">
                    {intel.content.postsPer30d}
...
export function AudienceSection({ gold }: SectionProps) {
  /*
    ...
    <div className="flex flex-col gap-4">
      ...
      <VizCard
        ...
        subtitle={q
          ...
          : undefined}
      >
        {q && (q.authenticity !== null || q.audienceQuality !== null || q.followerQuality !== null) ? (
          <>
            <div className="grid gap-2.5 mb-3" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(150px,1fr))' }}>
              <StatTile
              ...
              <StatTile
                ...
                value={q.audienceQuality === null ? NOT_MEASURED : `${q.audienceQuality} / 100`}
              />
              <StatTile
            ...
            <p className="text-[9.5px] mt-2.5 leading-[1.5]" style={{ color: T.t4 }}>
              ...
              {q.audienceQuality !== null && q.followerQuality !== null
                && q.audienceQuality === q.followerQuality
...
export function BrandFitSection({ match, matchScoreable, orgSlug }: SectionProps) {
  return (
    <div className="flex flex-col gap-4">
      <VizCard
        title="Brand Match"
        subtitle={
          matchScoreable === false
            ? 'Belum ada Brand Profile untuk workspace ini'
            : 'Skor creator ini terhadap Brand Profile yang tersimpan'
        }
      >
        {matchScoreable === null ? (
          <p className="text-[12px]" style={{ color: T.t3 }}>Menghitung…</p>
        ) : matchScoreable === false ? (
          <div className="flex flex-col gap-2.5">
            <p className="text-[12px] leading-relaxed" style={{ color: T.t2 }}>
              Brand Match membandingkan creator ini dengan kategori, audiens dan kata kunci brand
              kamu. Selama Brand Profile belum diisi, tidak ada pembanding — jadi tidak ada skor
              yang ditampilkan, bukan skor yang dikarang.
            </p>
            <a
              href={`${tabHref(orgSlug, 'settings')}&view=discover`}
              style={PJ}
              className="self-start inline-flex items-center gap-1 rounded-lg border text-[11.5px] font-bold px-3 h-8 bg-[#327488] border-[#327488] text-white hover:bg-[#285D6E]"
            >
