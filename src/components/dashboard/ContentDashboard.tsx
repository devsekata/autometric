...
import {
  CONTENT_OVERVIEW_KPIS, IG_POST_TYPE_PERF, POST_TYPE_INSIGHT,
  CONTENT_VOLUME_WEEKS, CONTENT_VOLUME_INSIGHT, TOP_POSTS_TABLE,
  TIKTOK_COMPLETION_DIST, TIKTOK_COMPLETION_INSIGHT,
  REEL_WATCH_BY_DURATION, REEL_WATCH_INSIGHT,
...
const FORMAT_ICON: Record<string, string> = {
  Reel: 'movie', Video: 'smart_display', Carousel: 'collections', Image: 'image',
}
...
export default function ContentDashboard() {
  ...
  const rows = useMemo(
    () => (format === 'All Formats' ? TOP_POSTS_TABLE : TOP_POSTS_TABLE.filter(r => r.format === format)),
    [format],
  )
      ...
      {() => (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            {CONTENT_OVERVIEW_KPIS.map((k, i) => <FlexKpiCard key={k.key} kpi={k} color={PALETTE[i % PALETTE.length]} />)}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
            <Card className="flex flex-col">
              ...
              <div className="px-4 pb-4 pt-3">
                <HBars items={IG_POST_TYPE_PERF.map(p => ({
                  label: p.label, value: p.reach, display: fmtNum(p.reach), color: p.color,
                }))} />
              </div>
              <div className="mx-4 mb-4 mt-auto">
                <Callout tone="success" title="Reels Dominate">{POST_TYPE_INSIGHT}</Callout>
              </div>
            </Card>
            ...
            <Card className="flex flex-col">
              ...
              <div className="px-4 pb-4 pt-3 flex-1 flex items-end">
                <BarChart height={200} bars={CONTENT_VOLUME_WEEKS.map(w => ({
                  label: w.label, value: w.value, display: String(w.value),
                  color: w.value < 32 ? '#d97a7a' : PALETTE[1],
                }))} />
              </div>
              <div className="mx-4 mb-4 mt-auto">
                <Callout tone="warning" title="Consistency Gap">{CONTENT_VOLUME_INSIGHT}</Callout>
              </div>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <Card className="flex flex-col">
              ...
              <div className="px-4 pb-4 pt-3 flex-1 flex items-end">
                <BarChart height={200} bars={TIKTOK_COMPLETION_DIST.map((d, i) => ({
                  label: d.label, value: d.value, display: `${d.value}%`, color: PALETTE[i % PALETTE.length],
                }))} />
              </div>
              <div className="mx-4 mb-4 mt-auto">
                <Callout tone="success" title="Strong Retention">{TIKTOK_COMPLETION_INSIGHT}</Callout>
              </div>
            </Card>
            ...
            <Card className="flex flex-col">
              ...
              <div className="px-4 pb-4 pt-3 flex-1 flex items-end">
                <BarChart height={200} bars={REEL_WATCH_BY_DURATION.map(d => ({
                  label: d.label, value: d.value, display: `${d.value}%`,
                  color: d.value >= 60 ? '#5fa783' : d.value >= 40 ? '#e0a458' : '#d97a7a',
                }))} />
              </div>
              <div className="mx-4 mb-4 mt-auto">
                <Callout tone="info" emoji="💡" title="Sweet Spot">{REEL_WATCH_INSIGHT}</Callout>
              </div>
            </Card>
          </div>
        </>
