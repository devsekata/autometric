...
import {
  OVERVIEW_KPIS, ENGAGEMENT_OVER_TIME, TREND_METRICS, TREND_LABELS,
  PLATFORM_REACH_SHARE, BRAND_MATRIX, CONTENT_ATTRIBUTES,
  CONTENT_ATTR_FINDING, POSTING_WINDOW_INSIGHT, POSTING_HEATMAP, HEATMAP_DAYS,
  HEATMAP_TIME_LABELS, PLATFORM_META, PALETTE, fmtNum,
  type TrendMetric,
} from './data'
...
const TOTAL_REACH = PLATFORM_REACH_SHARE.reduce((s, p) => s + p.value, 0)
      ...
      {() => (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 mb-3">
            {OVERVIEW_KPIS.map((k, i) => <FlexKpiCard key={k.key} kpi={k} color={PALETTE[i % PALETTE.length]} />)}
          </div>
            ...
            <Card span="col-span-12 lg:col-span-8">
              <div className="flex items-start justify-between px-4 pt-3.5 pb-2 flex-wrap gap-2">
                <div>
                  ...
                  <p className="text-[11px] text-[#9ca3af] mt-0.5">{metric} by brand · last 12 weeks</p>
                </div>
              ...
              <div className="px-4 pb-3 pt-1">
                <MultiLineChart series={ENGAGEMENT_OVER_TIME[metric]} labels={TREND_LABELS} height={210} />
              </div>
              <div className="flex items-center gap-4 px-4 pb-4 flex-wrap">
                {ENGAGEMENT_OVER_TIME[metric].map(s => (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            ...
            <Card className="flex flex-col">
              <div className="flex items-center justify-between px-5 pt-4 pb-1 gap-2">
                ...
                <span style={PJ} className="text-[11px] font-semibold text-[#3d8a5f] bg-[#eaf5ef] px-2.5 py-1 rounded-full">extra_attribute schema</span>
              </div>
              ...
              <div className="flex items-start gap-2.5 mx-5 mb-5 mt-auto px-4 py-3.5 rounded-xl bg-[#f3f1fb] border border-[#e7e3f7]">
                ...
                <div>
                  ...
                  <p className="text-[13px] leading-relaxed text-[#4b4563]">{CONTENT_ATTR_FINDING}</p>
                </div>
              </div>
            </Card>
            ...
            <Card className="flex flex-col">
              ...
              <div className="flex items-start gap-2.5 mx-5 mb-5 mt-auto px-4 py-3.5 rounded-xl bg-[#edf6ef] border border-[#d7ebdb]">
                ...
                <div>
                  ...
                  <p className="text-[13px] leading-relaxed text-[#4b5563]">{POSTING_WINDOW_INSIGHT}</p>
                </div>
              </div>
            </Card>
          </div>
