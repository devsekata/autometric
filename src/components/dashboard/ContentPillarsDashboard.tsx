...
interface Pillar { id: string; name: string; color: string; hashtags: string[] }

// Deterministic pseudo-metrics so a user-defined pillar gets stable numbers.
function pillarMetric(p: Pillar) {
  const seed = p.name.split('').reduce((s, c) => s + c.charCodeAt(0), 0)
  return { er: 3.5 + (seed % 80) / 10, posts: 6 + (seed % 40) }
}

export default function ContentPillarsDashboard() {
  ...
  const addPillar = () => {
    ...
    setPillars([...pillars, { id: crypto.randomUUID(), name: draftName.trim(), color: draftColor, hashtags: draftTags }])
    setDraftName(''); setDraftTags([]); setComparisonRun(false)
    setDraftColor(PILLAR_COLORS[(pillars.length + 1) % PILLAR_COLORS.length])
  }
  const removePillar = (id: string) => { setPillars(pillars.filter(p => p.id !== id)); setComparisonRun(false) }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <Card>
              <div className="px-5 pt-4 pb-1">
                ...
                <p className="text-[12.5px] text-[#9ca3af] mt-1 leading-relaxed">
                  Name each pillar, assign a color, and add relevant hashtags. Performance aggregates across all hashtags in that pillar — maps to <code className="text-[#6c4cd6] bg-[#f3f1fb] px-1 rounded">content_pillar</code> field in <code className="text-[#6c4cd6] bg-[#f3f1fb] px-1 rounded">*_post_extra_attribute</code> tables.
                </p>
              </div>
              ...
              <div className="m-5 mt-3 rounded-xl border border-dashed border-[#d1d5db] p-4">
                ...
                <button onClick={addPillar} style={PJ} className="inline-flex items-center gap-1.5 text-[13px] font-bold text-white bg-[#6c4cd6] rounded-lg px-4 py-2.5 hover:bg-[#5a3fc0]">
                    ...
                    <div className="flex flex-col gap-2">
                      {pillars.map(p => (
                        <div key={p.id} className="flex items-center gap-2.5 rounded-lg border border-[#eef0f2] px-3 py-2.5">
                          ...
                          <button onClick={() => removePillar(p.id)} className="material-symbols-outlined text-[16px] text-[#cbd1d8] hover:text-[#c2553f]">delete</button>
                        </div>
                      ))}
            ...
            <Card className="flex flex-col">
              {comparisonRun && pillars.length >= 2 ? (
                <>
                  <CardHead title="Pillar Performance Comparison" sub="Avg engagement rate aggregated per pillar" />
                  <div className="px-4 pb-5 pt-3 flex-1 flex flex-col justify-center">
                    <HBars items={pillars.map(p => {
                      const m = pillarMetric(p)
                      return { label: `${p.name} · ${m.posts} posts`, value: m.er, display: `${m.er.toFixed(1)}%`, color: p.color }
                    })} />
                  </div>
                </>
              ) : (
                <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-16">
                  ...
                  <p style={PJ} className="text-[14px] font-bold text-[#9ca3af]">No pillars yet</p>
