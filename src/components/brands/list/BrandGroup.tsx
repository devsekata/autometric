    ...
    <div className="border-b border-[#e5e7eb] last:border-0">
      ...
      <div className="flex items-center gap-2.5 px-6 py-2.5 cursor-pointer select-none bg-[#f9fafb]"
        ...
        <Link href={`/organizations/${orgSlug}/brands/${brand.id}/overview`}
          ...
          className="text-[13.5px] font-bold text-[#111827] hover:text-[#285D6E] hover:underline transition-colors">
          {brand.name}
        </Link>
      ...
      {open && (
        <div>
          ...
          {!allPlatformsFull && (
            ...
            >
              <span className="material-symbols-outlined text-[14px] text-[#d1d5db] group-hover/add:text-[#285D6E] transition-colors">add</span>
              <span style={PJB} className="text-[12px] text-[#d1d5db] group-hover/add:text-[#285D6E] transition-colors">Add channel</span>
            </button>
          )}
        </div>
      )}
