...
export default function ChartSelectionModal({
  ...
  const selected = 'border-[#327488] bg-[#EAF2F6] text-[#327488] ring-1 ring-[#327488]'
        ...
        {step === 3 && category === 'line' && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 max-h-[280px] overflow-y-auto">
              {LINE_METRICS.map(metric => {
                ...
                return (
                  ...
                  >
                    <span className={`w-5 h-5 rounded border flex items-center justify-center text-[11px] ${isSel ? 'bg-[#327488] border-[#327488] text-white' : 'border-[#cbd5e1]'}`}>
                      {isSel && '✓'}
                    </span>
            ...
            {customMetrics.length > 0 ? (
                  ...
                  {customMetrics.map(cm => {
                    ...
                    return (
                      <button key={cm.id} onClick={() => !disabled && toggleLine(cm.id)} disabled={disabled} style={PJ}
                        ...
                        <span className={`w-5 h-5 rounded border flex items-center justify-center text-[11px] ${isSel ? 'bg-[#327488] border-[#327488] text-white' : 'border-[#cbd5e1]'}`}>{isSel && '✓'}</span>
              ...
              <button onClick={() => setCmOpen(true)} className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-lg border border-dashed border-[#d7dde3] text-[11.5px] font-semibold text-[#94a3b8] hover:border-[#327488] hover:text-[#327488] hover:bg-[#EAF2F6] transition-all">
            ...
            <button onClick={confirm} disabled={lineMetrics.length === 0} style={PJ}
              className={`w-full py-3 rounded-xl font-bold text-[13px] transition-all ${lineMetrics.length > 0 ? 'bg-[#327488] text-white hover:bg-[#285D6E]' : 'bg-[#f1f3f5] text-[#b6bcc4] cursor-not-allowed'}`}>
        ...
        {step === 4 && category === 'bar' && barCategory && (
          <div className="space-y-4">
                    ...
                    {availableCompetitors.map(c => {
                      ...
                      return (
                        <button key={c.id} onClick={() => toggleComp(c.id)} style={PJ}
                          ...
                          <span className={`w-4 h-4 rounded border flex items-center justify-center text-[10px] ${on ? 'bg-[#327488] border-[#327488] text-white' : 'border-[#cbd5e1]'}`}>
                            {on && '✓'}
                          </span>
            ...
            <button onClick={confirm} disabled={barMetrics.length === 0} style={PJ}
              className={`w-full py-3 rounded-xl font-bold text-[13px] transition-all ${barMetrics.length > 0 ? 'bg-[#327488] text-white hover:bg-[#285D6E]' : 'bg-[#f1f3f5] text-[#b6bcc4] cursor-not-allowed'}`}>
