...
function nextRunLabel(times: ScheduleTime[], isActive: boolean): string {
  ...
  return `Berikutnya: ${fmtTime(next)} WIB · ${label} lagi`
}

export default function SchedulerSettingsPage() {
  const [times,     setTimes]     = useState<ScheduleTime[]>([{ hour: 2, minute: 0 }])
  ...
  useEffect(() => {
    fetch('/api/admin/scheduler/config')
      .then(r => r.json())
      .then(d => { setTimes(d.scheduleTimes ?? [{ hour: 2, minute: 0 }]); setIsActive(d.isActive ?? true) })
      .catch(() => {})
  ...
  async function save() {
    ...
    try {
      const res = await fetch('/api/admin/scheduler/config', {
        ...
        body: JSON.stringify({ scheduleTimes: times, isActive }),
      })
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <div className="mb-8">
        <h1 className="text-[28px] font-bold text-[#0f172a] tracking-[-0.03em]" style={PJB}>Scheduler</h1>
        <p className="text-[13.5px] text-[#94a3b8] mt-1" style={PJB}>Konfigurasi jadwal sync otomatis (WIB)</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[#f1f5f9]">
            <p className="text-[13px] font-bold text-[#0f172a]" style={PJB}>Jadwal Run</p>
            <p className="text-[12px] text-[#94a3b8] mt-0.5" style={PJB}>Scheduler akan jalan di setiap waktu yang dikonfigurasi (WIB)</p>
          </div>
          <div className="px-5 py-4 space-y-2.5">
              ...
              <div key={idx} className="flex items-center justify-between px-4 py-3 bg-[#f8fafc] rounded-xl border border-[#e2e8f0]">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-white border border-[#e2e8f0] flex items-center justify-center flex-shrink-0">
                    <span className="material-symbols-outlined text-[15px] text-[#3d7e96]">schedule</span>
                  </div>
                  <div>
            ...
            {adding ? (
              <div className="flex items-center gap-2 pt-1">
                <div className="flex items-center gap-1.5 flex-1 px-4 py-2.5 bg-[#f8fafc] rounded-xl border border-[#3d7e96]">
              ...
              <button onClick={openAdd} style={PJB}
                className="flex items-center gap-1.5 text-[13px] font-semibold text-[#3d7e96] hover:text-[#2d6a80] transition-colors pt-1">
          ...
          <div className="flex items-center justify-between">
            <div className="min-w-0 mr-4">
              ...
              <p className="text-[12.5px] mt-0.5" style={PJB}>
                {runState === 'running' && <span className="text-[#3b82f6]">Sedang sync semua akun…</span>}
                {runState === 'done' && runResult && (
                  <span className="text-[#166534]">
                    Selesai · {runResult.accounts} akun · {runResult.success} berhasil
                ...
                {runState === 'idle' && <span className="text-[#94a3b8]">Jalankan sync manual untuk semua akun</span>}
              </p>
            </div>
        ...
        <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-2xl px-5 py-4 space-y-3">
          ...
          <div className="space-y-2.5">
            <div className="flex items-start gap-2.5">
              <div className="w-5 h-5 rounded-full bg-[#dbeafe] flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="material-symbols-outlined text-[11px] text-[#1d4ed8]">computer</span>
              </div>
              <p className="text-[12.5px] text-[#64748b]" style={PJB}>
                <span className="font-semibold text-[#334155]">Lokal (dev):</span> node-cron berjalan di dalam app dan otomatis trigger sesuai jadwal di atas.
              </p>
            </div>
            <div className="flex items-start gap-2.5">
              <div className="w-5 h-5 rounded-full bg-[#dcfce7] flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="material-symbols-outlined text-[11px] text-[#166534]">cloud</span>
              </div>
              <p className="text-[12.5px] text-[#64748b]" style={PJB}>
          ...
          <code className="block text-[11px] text-[#64748b] bg-white border border-[#e2e8f0] rounded-xl px-3 py-2 leading-relaxed whitespace-pre">
            {'* * * * * curl -s -X POST https://domain.com/api/scheduler/run \\\n  -H "Authorization: Bearer $SCHEDULER_SECRET"'}
          </code>
        </div>
