...
import { getTiktokData, type PlatformParam } from '@/lib/dashboard/tiktok'
...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    const data = await getTiktokData(orgId, platform, days, brandId, custom)
    return NextResponse.json(data)
  } catch (err) {
    console.error('[GET /api/organizations/[id]/dashboard/tiktok]', err)
