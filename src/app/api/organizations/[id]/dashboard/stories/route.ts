...
import { getStoriesData, type PlatformParam } from '@/lib/dashboard/stories'
...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    const data = await getStoriesData(orgId, platform, days, brandId, custom)
    return NextResponse.json(data)
  } catch (err) {
    console.error('[GET /api/organizations/[id]/dashboard/stories]', err)
