 ...
 *
 * Ported from `origin/engkol_v2`. On this branch the engines downstream are not
 * wired yet, so today this route only reads and writes `public.brand_profile`.
 * `orgId` is the agency id (`public.agencies.id`) that `requireOrgMemberById`
 * authorised; a body cannot choose the organization, and cannot set brandId.
 *
