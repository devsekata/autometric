...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    const org = await updateOrg(id, name, userId)
    if (!org) return NextResponse.json({ error: 'Organization not found.' }, { status: 404 })
  ...
  try {
    ...
    if (brandCount > 0) {
      return NextResponse.json(
        {
          error: `This organization still has ${brandCount} brand${brandCount !== 1 ? 's' : ''}. Delete them first, then delete the organization.`,
    ...
    const deleted = await softDeleteOrg(id)
